...
// Date: May 11, 2026 — Upgraded from v2.5.2 (April 1, 2026)
//
// v3.0.0 Adversarial Fix Registry:
// [FATAL]  AF-SEC-02 — score() async integration: safeCERTUSScore in index.html
//          must await CERTUS.score(...). Engine-side contract documented below.
// [CRIT]   PDE-GAP-014 — Abstraction Bargain declared: MODEL_LIMITATIONS block
//          documents all discarded properties and their failure classes.
// [HIGH]   PDE-VUL-003 — DCI weights calibration path: FCL logging hooks added;
//          _logFCLEntry() records scoring outcomes against ground truth.
// [HIGH]   PDE-OVE-002 — bayesianUpdate domain declaration added per McCarthy (1960).
//          Domain: prior ∈ [0,1], likelihood ∈ [0,1]. Out-of-domain clamped.
// [HIGH]   PDE-LOO-001 — EPISTEMIC_CEILING now cumulative across all appeals.
//          _cumulativeAppealBoost tracked per report; total clamped to 0.95.
// [HIGH]   PDE-VUL-004 — PERCEPTUAL_HASH_THRESHOLD surfaced to THRESHOLDS config.
// [HIGH]   PDE-BLI-001 — NLP_CONFIG language limitation documented with mitigation
//          guidance for non-English deployments.
// [MED]    PDE-FAI-001 — _degradationReasons capped at 100 entries (LRU eviction).
// [MED]    AF-REG-01 — Mock emergency data extracted to MOCK_EMERGENCY_CONFIG;
//          production deployments must override with live facility databases.
// [MED]    AF-INFRA-03 — healthCheck() now callable via /api/health endpoint pattern.
// [CAL]    FTT-CAL-07 — @complexity annotations added to all scoring functions.
// [CAL]    FTT-ECI-01 — STP integrity seal added to score output via _sealResult().
// [CAL]    FTT-ECI-03 — Input hash verification via _hashReportInput() before scoring.
// [CAL]    FTT-14    — Abstraction Bargain declared in MODEL_LIMITATIONS.
// [CAL]    FTT-4     — bayesianUpdate domain declared; out-of-domain inputs clamped.

const CERTUS = {

  // ── VERSION ────────────────────────────────────────────────────────────────
  VERSION: '3.0.0',
  CANARY_VERSION: '3.0.0-beta',
  ...
  THRESHOLDS: {
    ...
    // PDE-VUL-004 FIX: perceptual hash threshold surfaced to config.
    // Previously hardcoded at 0.95 inside _findDuplicatePhotos.
    ...
    // PDE-FAI-001 FIX: degradation reasons capped at 100 entries.
  ...
  // PDE-LOO-001 FIX: cumulative appeal boost tracker per report.
  // Prevents sequential appeals from exceeding EPISTEMIC_CEILING.
  _cumulativeAppealBoost: new Map(),
  // FCL calibration entries
  _fclEntries: [],
  _fclMaxEntries: 500,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  _logFCLEntry(scoringResult, groundTruth) {
  ...
  _hashReportInput(report) {
    ...
    let h = 0;
    for (let i = 0; i < payload.length; i++) { h = ((h << 5) - h) + payload.charCodeAt(i); h |= 0; }
    return `inp-${Math.abs(h).toString(16)}`;
  },
  ...
  getDataSharingDisclosure() { return { recipients: Object.entries(this.DATA_RECIPIENTS).map(([key, r]) => ({ ...r, can_opt_out: r.opt_out })), total_recipients: Object.keys(this.DATA_RECIPIENTS).length, last_updated: new Date().toISOString() }; },
  ...
  async submitBatch(sessionId) {
    const batch = this._batchReports.get(sessionId); if (!batch) return { error: 'No batch found' };
    const results = []; for (const report of batch.reports) { const result = await this.score(report); results.push(result); }
    await this._logAuditEvent({ type: 'BATCH_SUBMITTED', batch_id: sessionId, report_count: batch.reports.length });
  ...
  recognizeOfflineVoice(audioSample, language = 'en') {
    ...
    return { detected: keywords[0], confidence: 0.7, offline: true, stub: true, stub_warning: 'Returns first keyword unconditionally. Audio sample is not analysed.' };
  },
  ...
  _extractDamageFromWitness(statement) {
    ...
    let ds = 0, dl = 'partial';
    for (const kw of this.NLP_CONFIG.damageKeywords.minimal) if (text.includes(kw)) ds -= 0.3;
    for (const kw of this.NLP_CONFIG.damageKeywords.partial) if (text.includes(kw)) ds += 0.5;
    for (const kw of this.NLP_CONFIG.damageKeywords.complete) if (text.includes(kw)) ds += 1.0;
    ds = Math.max(0, Math.min(1, (ds + 0.5) / 2));
    if (ds >= 0.7) dl = 'complete'; else if (ds >= 0.4) dl = 'partial'; else dl = 'minimal';
  ...
  async _initializeStorage() {
    if (typeof indexedDB !== 'undefined') {
      return new Promise((resolve, reject) => {
        const req = indexedDB.open('CERTUS_DB', 1);
        ...
        req.onupgradeneeded = (e) => { const db = e.target.result; if (!db.objectStoreNames.contains('appeals')) db.createObjectStore('appeals', { keyPath: 'id' }); if (!db.objectStoreNames.contains('audit')) db.createObjectStore('audit', { keyPath: 'timestamp' }); if (!db.objectStoreNames.contains('reputation')) db.createObjectStore('reputation', { keyPath: 'reporter_id' }); if (!db.objectStoreNames.contains('shards')) db.createObjectStore('shards', { keyPath: 'id' }); };
      });
    }
  ...
  async storeAppeal(ar) { if (!this._storage) await this._initializeStorage(); if (this._storage.type === 'indexeddb') { const tx = this._storage.db.transaction(['appeals'], 'readwrite'); tx.objectStore('appeals').put(ar); return new Promise((res, rej) => { tx.oncomplete = () => res(); tx.onerror = () => rej(tx.error); }); } else { const a = this._storage.get('appeals') || []; a.push(ar); this._storage.set('appeals', a); } if (this._supabaseClient && this._supabaseClient.from) { try { await this._supabaseClient.from('appeals').insert(ar); } catch (err) {} } },

  async updateReputationStorage(rid, rep) { if (!this._storage) await this._initializeStorage(); if (this._storage.type === 'indexeddb') { const tx = this._storage.db.transaction(['reputation'], 'readwrite'); tx.objectStore('reputation').put({ reporter_id: rid, ...rep }); return new Promise((res, rej) => { tx.oncomplete = () => res(); tx.onerror = () => rej(tx.error); }); } else { const rs = this._storage.get('reputation') || {}; rs[rid] = rep; this._storage.set('reputation', rs); } if (this._supabaseClient && this._supabaseClient.from) { try { await this._supabaseClient.from('reputation').upsert({ reporter_id: rid, ...rep, updated_at: new Date().toISOString() }); } catch (err) {} } },
  ...
  async initialize(supabaseUrl = null, supabaseAnonKey = null) { await this._initializeStorage(); if (supabaseUrl && supabaseAnonKey) this.initSupabase(supabaseUrl, supabaseAnonKey); await this._logAuditEvent({ type: 'ENGINE_INITIALIZED', version: this.VERSION, storage_type: this._storage?.type, supabase_available: !!this._supabaseClient }); return { success: true, version: this.VERSION, storage: this._storage?.type, supabase: !!this._supabaseClient }; },
  ...
  async processAppeal(originalReport, newEvidence, ipAddress, context = {}) {
    ...
    // PDE-LOO-001 FIX: cumulative ceiling tracking
  ...
  async score(report, nearbyReports = [], isRealModel = false, context = {}) {
    ...
    const inputHash = this._hashReportInput(report);
    ...
    if (requiresHumanReview && !hasValidHR) {
      ...
      // Log FCL entry with null ground truth (outcome pending)
      this._logFCLEntry({ _reportUuid: reportUuid, dci, tier: 'review', dci_validity_status: um.validity_status, dci_pes: PES.value, dci_cor: COR.value, dci_tfr: TFR.value, dci_cci: CCI.value }, null);
    ...
    const er = this.getEmergencyResources(report, location);
    const sd = this.getShareData(report, vc);

    // CAL FTT-ECI-01: integrity seal over the result
    const seal = await this._sealResult({ dci, tier, dci_validity_status: um.validity_status }, reportUuid);

    // FCL entry: log scoring outcome
    this._logFCLEntry({ _reportUuid: reportUuid, dci, tier, dci_validity_status: um.validity_status, dci_pes: PES.value, dci_cor: COR.value, dci_tfr: TFR.value, dci_cci: CCI.value }, context.ground_truth || null);
    ...
    return {
      ...
      dci_flags: { pes_gated: PES.gated, pes_inferential: PES.measurement_class === 'INFERENTIAL', pes_missing: !PES.evaluable, cor_no_evidence: COR.signal_type === 'NO_EVIDENCE', cor_contradiction: COR.signal_type === 'CONTRADICTION', cci_flagged: CCI.flagged, tfr_status: TFR.freshness_status, correlated_failure: cf.correlated },
...
if (typeof window !== 'undefined') { window.CERTUS = CERTUS; CERTUS.initialize().catch(console.warn); }
